...
class StudentApp(tk.Tk):
    ...
    def _build_widgets(self) -> None:
        ...
        ttk.Entry(form, textvariable=self.name_var, width=15).grid(row=1, column=0)
        ttk.Entry(form, textvariable=self.age_var, width=5).grid(row=1, column=1)
        ttk.Entry(form, textvariable=self.city_var, width=10).grid(row=1, column=2)
        ttk.Entry(form, textvariable=self.index_var, width=10).grid(row=1, column=3)
        ttk.Entry(form, textvariable=self.avg_var, width=7).grid(row=1, column=4)

        ttk.Button(form, text="Add", command=self._on_add).grid(row=1, column=5)
        ...
        ttk.Button(
            search_frame,
            text="Professors",
            command=self._open_professors_window
    ...
    def _open_professors_window(self):

        window = tk.Toplevel(self)
        window.title("Professors")
        window.geometry("600x400")

        ProfessorWindow(window, self.db)
...
class ProfessorWindow:
    ...
    def _build_widgets(self):
        ...
        ttk.Label(form, text="Name").grid(row=0, column=0)
        ttk.Label(form, text="Subject").grid(row=0, column=1)
        ttk.Label(form, text="Department").grid(row=0, column=2)
        ttk.Label(form, text="City").grid(row=0, column=3)
        ...
        ttk.Entry(form, textvariable=self.name).grid(row=1, column=0)
        ttk.Entry(form, textvariable=self.subject).grid(row=1, column=1)
        ttk.Entry(form, textvariable=self.department).grid(row=1, column=2)
        ttk.Entry(form, textvariable=self.city).grid(row=1, column=3)

        ttk.Button(form, text="Add", command=self._add_professor).grid(row=1, column=4)

        self.tree = ttk.Treeview(
            self.root,
            columns=("name", "subject", "department", "city"),
            show="headings"
        )
    ...
    def _add_professor(self):
        ...
        self.db.add_professor(
            name=self.name.get(),
            subject=self.subject.get(),
            department=self.department.get(),
            place_of_residence=self.city.get()
        )
    ...
    def _refresh_list(self):
        ...
        for p in self.db.list_professors():
